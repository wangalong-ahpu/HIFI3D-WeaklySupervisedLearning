...
class LYMH_TestDataset(Dataset):
    def __init__(self, testpath, crop_size=224):
        ...
        '''
        testpath = os.path.join(testpath, "image/1/")
...
class LYMH_TrainDataset(Dataset):
    def __init__(self, K, image_size, scale, trans_scale = 0, isTemporal=False, isEval=False, isSingle=False, kpt_num=235, train_path=None):
        '''
        ...
        self.segfolder = os.path.join(train_path, 'seg'+'/')
        datafile = os.path.join(train_path, 'train_list_kpt'+str(kpt_num)+'.npy')
        if isEval:
            datafile = os.path.join(train_path, 'train_list_kpt'+str(kpt_num)+'.npy')
    ...
    def load_mask(self, maskpath, h, w):
        ...
        if os.path.isfile(maskpath):
            ...
            mask = np.zeros_like(vis_parsing_anno).astype(float)
            # for i in range(1, 16):

...
class SRenderY(nn.Module):
    ...
    def world2uv_custom(self, vertices, tex_size=None, faces=None, uvcoords=None, uvfaces=None):
        ...
        if uvcoords is None and uvfaces is None and faces is None:
            face_vertices = util.face_vertices(vertices,
                                               self.faces.expand(batch_size, -1, -1))  # torch.Size([1, 9976, 3, 3])
            ...
            uv_vertices = self.uv_rasterizer(uvcoords.expand(batch_size, -1, -1), uvfaces.expand(batch_size, -1, -1),
                                             face_vertices)[:, :3]
...
class SRenderY_2(nn.Module):
    def __init__(self, image_size, obj_filename, uv_size=256, rasterizer_type='pytorch3d'):
        super(SRenderY_2, self).__init__()
        self.image_size = image_size
        self.uv_size = uv_size
        if rasterizer_type == 'pytorch3d':
            self.rasterizer = Pytorch3dRasterizer(image_size)
